...
- Versioning policy 📐: the SDK is unreleased, and the server does not return `X-Wizard-Api-Version` either

## 2. Python SDK (the primary SDK, eval/rollout side) ⚠️

**What is actually covered today** (`sdk/python/wizard/__init__.py`, a single hand-written file):
...
✅ Error tiering: WizardAPIError / WizardConnectionError

📐 Unimplemented: pty, exec_stream, ports.expose, volumes, fork, start,
   set_lifecycle, files.upload_dir/download_dir, wizard.aio (the async stack),
...
Package name: `wizard-sdk` (import `wizard`). Dependency: `httpx`.
...
# Environment-variable fallback: WIZARD_API_KEY / WIZARD_BASE_URL
...
`wizard.aio` mirrors the same interface shape (`AsyncWizardClient` / `AsyncSandbox`), sharing the
generated layer and model definitions. The primary form for high-concurrency rollout scenarios.

### 2.3 eval batch helper 📐

> `wizard.batch.run_batch` **does not exist**. What follows is design intent.
...
- Error mapping: `WizardAPIError` as the base class, deriving `SandboxNotFound`, `QuotaExceeded`, `NoCapacity`… by code
...
Package name: `@wizard/sdk` (an npm org scope, dodging the taken bare name). Runtime: Node 18+ /
...
`~/.config/wizard/config.yaml` (multi-profile: endpoint + key).

### 4.1 Command surface ⚠️

What is actually implemented (`wizard --help`):
...
wizard events SBX             # history; `-f [SBX] [--label k=v]` follows the live stream (SSE)
wizard kill SBX [--force]
wizard pause SBX / wizard resume SBX   # freeze and wake the same sandbox
wizard run --snapshot SNAP           # create from a snapshot: a new sandbox each time it is called
wizard build  --tag REF [--file Dockerfile] [CONTEXT]   # build a template on the platform
...
- Exit codes distinguish "should the script retry":
  ...
  help**), `70` the platform explicitly refused, `125` usage error. `wizard exec` passes the
  remote exit code through
- Environment variables: `WIZARD_BASE_URL`, `WIZARD_API_KEY`, `WIZARD_TIMEOUT` (a Go duration, 15m by default)
...
- Local terminal in raw mode wired to WS PTY frames, SIGWINCH → resize frame, Ctrl-P Ctrl-Q
  to detach (the session is kept for 60s so `wizard attach SBX` can reconnect)
