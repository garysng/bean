...
| VMM mount namespace | ✅ | `--fc-mount-namespace`, **on by default**. Held back at first on the expectation that wizard's device-mapper rootfs would stop being openable inside one. That was wrong: a booted guest has a working `eth0` and its own mnt, pid and net namespaces at once |
...
| Per-port access control | 📐 | Any port on a sandbox is reachable by anything that can reach wizard-proxy. A sandbox must not be given a port it would not want its caller to see (api-design.md §3.4) |
...
`wizard_node_disk_{free,used}_bytes`, `UpdateNodeStatus`'s `disk_used_mib` (moved
...
**A guest's `sync` did not put its writes on the device, and that was wizard's bug.** It is fixed;
...
and its data block all zeros. Two defects, both wizard's, and neither was the guest's fault.
...
The Go layer reader passed every test in its package and could not read a single layer wizard
...
1. **The payload is not at offset 0.** wizard seals with `overlaybd-commit -z -t`, and `-t`
...
`wizard_node_create_phase_seconds` with a `runtime` label, so every phase read `n/a` while the
