...
同一 backend 不同次之间的差异比两者之间的差异还大。

这一条值得多说,因为它和 overlaybd 出名的那个卖点正好相反。原因是**上面测的这条路径
仍然是「先转换再创建」** —— 下载每一层、逐层封装、然后组设备。首次使用时它做的事
比拍平**更多**;它赢在第二个镜像和磁盘上,不赢在首次 create。
...
所以 **lazy pull 是镜像的属性,不是节点 flag 的属性**。在只喂普通 registry 镜像的节点上
开 `--fc-overlaybd-lazy-pull` 什么也做不到:`lowersFor` 现在会**显式拒绝**这种 create,
而不是拼出一个 overlaybd 打开后找不到 LSMT 结构的 config。decisions §3.1 里实测的
7ms 挂载和 19.6% 传输量,对的是一个**已经转换并推送过**的 blob —— 那一步正是这个节点没做的。

**所以冷路径没变,并且在镜像以 overlaybd 形式发布之前都不会变。** 那是个流水线问题
(集中转换一次、推送、每个节点 range-read),不是 provider 的问题;
如果目标是冷启动延迟,那才是下一件该做的事。在那之前 prewarm 仍然是必需的,
和拍平路径完全一样。
