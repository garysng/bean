...
| Rootfs 按需拉取 | overlaybd | ✅ 两条传输都支持,已在真机验证。TCMU 下发 range 请求的是 overlaybd 守护进程;ublk 下(`--fc-ublk`)是 noded 自己读层格式。ublk 路径实测:guest 从一个不在本地磁盘的层 358ms 起来,最多读了 5.1 MiB 层的 60%。前提是那一层已经是封好的 overlaybd 层(普通 OCI 层没有块索引),且对象存储允许匿名 GET |
