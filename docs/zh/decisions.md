...
排队则让延迟可预测。已实装(`--create-wait`,默认 0),实测 30 并发从
16/30 变成 **30/30**,而 wall 从 8s 到 13s —— **吞吐没变,只是把拒绝换成了延迟**,
这正是上面那条分析预测的结果。GitHub #19。

**只对创建并发排队。** 区别是时长而非严重程度:创建并发几秒自己空出来,
而 CPU/内存/磁盘按 sandbox 生命周期持有 —— 等十秒还是不够,
等待只会把一个快速清晰的拒绝变成缓慢的、内容相同的拒绝。

**拒绝要说清是哪个资源。** 这条是被自己坑出来的:同一个 burst 在三种配置下
分别被磁盘、CPU、创建并发限制到 5/8/16,而报错一模一样 ——
我因此把「16」误判为 `max_creates`,实际是核数。未归因的容量错误最可能的
反应是去调错的那个上限,而那不起作用。所以现在会报出每个资源阻塞了多少节点、
最接近的节点差多少。
