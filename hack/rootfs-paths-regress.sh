...
check() {
	...
	if [ -z "$sbx" ]; then
		...
		"$BIN/wizard" run --image-ref "$IMAGE" --disk-mib 2048 2>&1 | tail -3 | sed 's/^/    /'
		fail=$((fail + 1))
		kill_all
		return
	fi
	...
	# outside.
	local out
	out=$("$BIN/wizard" exec "$sbx" -- sh -c 'cat /etc/alpine-release' 2>&1 | head -1)
	if [ "$out" = "3.20.10" ]; then
		...
		grep -acE "EXT4-fs error|I/O error" "/var/lib/wizard/sandboxes/$sbx/console.log" 2>/dev/null |
	...
	local wout
	wout=$("$BIN/wizard" exec "$sbx" -- sh -c 'echo probe > /root/w.txt && cat /root/w.txt' 2>&1 | head -1)
	if [ "$wout" = "probe" ]; then
	...
	kill_all
	say "  leaks: ublk=$(ls -1 /dev/ublkb* 2>/dev/null | wc -l) dm=$(dmsetup ls 2>/dev/null | grep -c wizard || true)"
}
