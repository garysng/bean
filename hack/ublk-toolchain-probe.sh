#!/usr/bin/env bash
# Reports whether this host can build wizard, and what is missing if not.
#
...
say ""
say "== wizard binaries already here =="
for b in noded wizard wizard-api wizard-proxy wizardd; do
	p=$(command -v "$b" 2>/dev/null)
	if [ -n "$p" ]; then
		say "$b: $p"
	fi
done
ls -1 /usr/local/bin 2>/dev/null | grep -E '^(wizard|noded|wizardd)' | sed 's/^/local: /'

say ""
say "== previous run artifacts =="
ls -1d /var/lib/wizard/* 2>/dev/null | head -10
