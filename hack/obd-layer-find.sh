...
# able to open.
#
# It matters that these were produced by upstream's `overlaybd-commit`, not by wizard's
...
for d in /var/lib/wizard/layers /var/lib/wizard/images /var/lib/wizard/tplimages \
	/var/lib/wizard/tplimages2 /var/lib/wizard/p2images /var/lib/wizard/p3images; do
	[ -d "$d" ] || continue
...
# naming convention is wizard's and the format is not.
found=0
...
done < <(find /var/lib/wizard -maxdepth 3 -type f -size +8k 2>/dev/null)
