...
wipe_node() { rm -rf "$RUN" /var/lib/wizard/images /opt/overlaybd/registry_cache/*; }
...
measure() {  # measure <label> <noded flags>
  ...
  sbx=$(timeout 300 "$BIN/wizard" run --image "$IMAGE" --quiet 2>/dev/null)
  ...
  "$BIN/wizard" exec "$sbx" -- cat /etc/os-release >/dev/null 2>&1 && ok="guest-ok"
...
docker exec wizard-minio mc alias set local "$S3" "$WIZARD_S3_ACCESS_KEY" "$WIZARD_S3_SECRET_KEY" >/dev/null 2>&1
docker exec wizard-minio mc rb --force "local/$BUCKET" >/dev/null 2>&1
...
docker exec wizard-minio mc ls --recursive "local/$BUCKET" 2>/dev/null \
  | awk '{printf "      %s %s\n", $4, $6}' | head -6
