...
sleep 3
grep -oE 'rootfs via overlaybd.{0,90}' /tmp/wizardrun/noded.log | tail -1
...
say "sandbox: $SBX"
"$BIN/wizard" exec "$SBX" -- sh -c "echo $MARKER > /root/marker.txt; cat /root/marker.txt" 2>&1 | head -2
...
# written one.
"$BIN/wizard" exec "$SBX" -- sh -c "sync" >/dev/null 2>&1
"$BIN/wizard" exec "$SBX" -- sh -c "echo 3 > /proc/sys/vm/drop_caches" >/dev/null 2>&1
sleep 1
say "overlay allocated: $(du -B1 "/var/lib/wizard/sandboxes/$SBX/overlay.img" 2>/dev/null | cut -f1) bytes"
...
SNAP=$("$BIN/wizard" snapshot create "$SBX" --no-memory 2>&1 | awk '/^snap/ {print $1}')
if [ -z "$SNAP" ]; then
	say "snapshot failed:"
	"$BIN/wizard" snapshot create "$SBX" --no-memory 2>&1 | tail -4 | sed 's/^/  /'
	exit 1
fi
say "snapshot: $SNAP"
kill_all
...
SBX2=$("$BIN/wizard" run --snapshot "$SNAP" --disk-mib 2048 2>&1 | awk '/^sbx_/ {print $1}')
if [ -z "$SBX2" ]; then
	say "FAIL: restore produced no sandbox"
	"$BIN/wizard" run --snapshot "$SNAP" --disk-mib 2048 2>&1 | tail -4 | sed 's/^/  /'
	grep -oE 'level=ERROR.{0,170}' /tmp/wizardrun/noded.log | tail -2 | sed 's/^/  /'
	exit 1
fi
...
GOT=$("$BIN/wizard" exec "$SBX2" -- sh -c 'cat /root/marker.txt' 2>&1 | head -1)
if [ "$GOT" = "$MARKER" ]; then
	...
	say "  block-io errors in the guest: $(grep -acE 'EXT4-fs error|I/O error' "/var/lib/wizard/sandboxes/$SBX2/console.log" 2>/dev/null)"
	fail=1
fi

say ""
say "== 5. the restored sandbox is writable in its own right =="
W=$("$BIN/wizard" exec "$SBX2" -- sh -c 'echo second > /root/second.txt && cat /root/second.txt' 2>&1 | head -1)
if [ "$W" = "second" ]; then
