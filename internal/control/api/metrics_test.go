...
func TestMetricsRecordCreateAndExec(t *testing.T) {
	...
	for _, want := range []string{
		`wizard_sandbox_creates_total{outcome="success"} 1`,
		"wizard_sandbox_create_duration_seconds_count",
		"wizard_exec_duration_seconds_count",
		`wizard_sandboxes{state="RUNNING"} 1`,
		`wizard_events_total{type="sandbox.lifecycle.created"} 1`,
	} {
...
func TestMetricsStateGaugesReflectStore(t *testing.T) {
	...
	if body := scrape(t, env); !strings.Contains(body, `wizard_sandboxes{state="RUNNING"} 1`) {
	...
	if !strings.Contains(body, `wizard_sandboxes{state="STOPPED"} 1`) {
		t.Errorf("stopped gauge missing:\n%s", body)
	}
	if !strings.Contains(body, `wizard_sandboxes{state="RUNNING"} 0`) {
...
func TestMetricsRecordFailedCreate(t *testing.T) {
	...
	if body := scrape(t, env); !strings.Contains(body, `wizard_sandbox_creates_total{outcome="error"} 1`) {
...
func TestMetricsRegistryExposed(t *testing.T) {
	...
	srv.Metrics().IncCounter("wizard_custom_total", "Custom.", nil, 3)
	if body := scrape(t, env); !strings.Contains(body, "wizard_custom_total 3") {
