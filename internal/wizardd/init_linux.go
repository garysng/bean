...
// it needs no wizardd, no init system and no knowledge of the platform.
//
...
func PivotToRootfs(device string) error {
	...
	if err := unix.Mount(device, newRoot, "ext4", 0, ""); err != nil {
		return fmt.Errorf("wizardd: mount rootfs %s: %w", device, err)
	}
	...
	if err := os.MkdirAll(agentTarget, 0o755); err != nil {
		return fmt.Errorf("wizardd: create %s: %w", agentTarget, err)
	}
	if err := unix.Mount(agentMount, agentTarget, "", unix.MS_BIND, ""); err != nil {
		return fmt.Errorf("wizardd: bind agent disk: %w", err)
	}
	...
	for _, fs := range []string{"/proc", "/sys", "/dev"} {
		...
		if err := os.MkdirAll(target, 0o755); err != nil {
			return fmt.Errorf("wizardd: create %s: %w", target, err)
		}
		if err := unix.Mount(fs, target, "", unix.MS_MOVE, ""); err != nil {
			return fmt.Errorf("wizardd: move %s: %w", fs, err)
		}
	}
	...
	if err := unix.Chdir(newRoot); err != nil {
		return fmt.Errorf("wizardd: chdir %s: %w", newRoot, err)
	}
	if err := unix.Mount(newRoot, "/", "", unix.MS_MOVE, ""); err != nil {
		return fmt.Errorf("wizardd: move new root: %w", err)
	}
	if err := unix.Chroot("."); err != nil {
		return fmt.Errorf("wizardd: chroot: %w", err)
	}
	if err := unix.Chdir("/"); err != nil {
		return fmt.Errorf("wizardd: chdir /: %w", err)
	}
...
func mountPseudoFS() error {
	...
	for _, m := range mounts {
		if err := unix.Mount(m.source, m.target, m.fstype, m.flags, ""); err != nil {
			...
			return fmt.Errorf("wizardd: mount %s (is it present on the agent disk?): %w",
				m.target, err)
		}
	}
	return nil
...
func mountTmpfs() error {
	for _, m := range []struct{ target, opts string }{
		...
		if err := os.MkdirAll(m.target, 0o755); err != nil {
			return fmt.Errorf("wizardd: create %s: %w", m.target, err)
		}
		if err := unix.Mount("tmpfs", m.target, "tmpfs", 0, m.opts); err != nil {
			return fmt.Errorf("wizardd: mount %s: %w", m.target, err)
		}
	}
	return nil
}
