...
func WriteResolvConf(root, addr string) error {
	if err := ValidateResolver(addr); err != nil {
		return fmt.Errorf("wizardd: %w", err)
	}
	...
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		return fmt.Errorf("wizardd: create %s: %w", filepath.Dir(path), err)
	}
	...
	if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("wizardd: replace %s: %w", path, err)
	}

	content := "nameserver " + addr + "\n"
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		return fmt.Errorf("wizardd: write %s: %w", path, err)
	}
	return nil
}
