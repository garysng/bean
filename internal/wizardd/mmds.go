...
func (c *mmdsClient) read(ctx context.Context) (*mmdsDoc, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("wizardd: read metadata: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("wizardd: read metadata: unexpected status %d", resp.StatusCode)
	}

	body, err := io.ReadAll(io.LimitReader(resp.Body, 64<<10))
	if err != nil {
		return nil, fmt.Errorf("wizardd: read metadata body: %w", err)
	}
	var doc mmdsDoc
	if err := json.Unmarshal(body, &doc); err != nil {
		return nil, fmt.Errorf("wizardd: parse metadata: %w", err)
	}
	return &doc, nil
}
...
func (c *mmdsClient) sessionToken(ctx context.Context) (string, error) {
	...
	resp, err := c.http.Do(req)
	if err != nil {
		return "", fmt.Errorf("wizardd: metadata session token: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("wizardd: metadata session token: unexpected status %d",
			resp.StatusCode)
	}
	body, err := io.ReadAll(io.LimitReader(resp.Body, 4<<10))
	if err != nil {
		return "", fmt.Errorf("wizardd: metadata session token body: %w", err)
	}
	if len(body) == 0 {
		return "", fmt.Errorf("wizardd: metadata session token is empty")
	}
