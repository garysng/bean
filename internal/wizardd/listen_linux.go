...
func listenVsock(port uint32) (net.Listener, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("wizardd: vsock socket: %w", err)
	}

	sa := &unix.SockaddrVM{CID: unix.VMADDR_CID_ANY, Port: port}
	if err := unix.Bind(fd, sa); err != nil {
		unix.Close(fd)
		return nil, fmt.Errorf("wizardd: bind vsock port %d: %w", port, err)
	}
	if err := unix.Listen(fd, 128); err != nil {
		unix.Close(fd)
		return nil, fmt.Errorf("wizardd: listen vsock port %d: %w", port, err)
	}
