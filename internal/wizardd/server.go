// Package wizardd implements the in-sandbox daemon (wizardd) gRPC service.
package wizardd
...
func (s *Server) writeFileFrom(recv func() (*commonv1.WriteFileFrame, error)) (*commonv1.WriteFileResponse, error) {
	...
	tmpRel := filepath.Join(filepath.Dir(rel), fmt.Sprintf(".wizard-tmp-%s", randSuffix()))
...
func (s *Server) StartUserProcess(ctx context.Context, req *agentv1.StartUserProcessRequest) (*agentv1.StartUserProcessResponse, error) {
	...
	go func() {
		...
		fmt.Fprintf(s.logs, "\n[wizardd] user process exited: code=%d\n", exitCode)
		s.mu.Lock()
		s.userProc = nil
