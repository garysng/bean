...
func Listen(addr string) (net.Listener, error) {
	if port, ok := strings.CutPrefix(addr, "vsock:"); ok {
		...
		if err != nil {
			return nil, fmt.Errorf("wizardd: vsock port %q: %w", port, err)
		}
		if n == 0 {
			return nil, fmt.Errorf("wizardd: vsock port must be non-zero")
		}
	...
	if hostPort, ok := strings.CutPrefix(addr, "tcp:"); ok {
		if _, _, err := net.SplitHostPort(hostPort); err != nil {
			return nil, fmt.Errorf("wizardd: tcp address %q: %w", hostPort, err)
		}
	...
	if !strings.HasPrefix(addr, "/") {
		return nil, fmt.Errorf("wizardd: unusable listen address %q: a Unix socket "+
	...
	if err := os.MkdirAll(filepath.Dir(addr), 0o755); err != nil {
		return nil, fmt.Errorf("wizardd: create socket dir: %w", err)
	}
	if err := os.Remove(addr); err != nil && !os.IsNotExist(err) {
		return nil, fmt.Errorf("wizardd: remove stale socket: %w", err)
	}
