"""wizard SDK: minimal synchronous client for the wizard sandbox platform."""
...
__all__ = [
    "WizardClient", "Sandbox", "Snapshot", "ExecResult", "Event",
    "WizardAPIError", "WizardConnectionError",
]
...
class WizardConnectionError(WizardAPIError):
...
class Snapshot:
    ...
    base_id: str = ""
    chain_depth: int = 0
    _client: "WizardClient" = field(default=None, repr=False, compare=False)

    @classmethod
    def _from_json(cls, obj: Dict[str, Any], client: "WizardClient" = None) -> "Snapshot":
        return cls(
...
class Sandbox:
    ...
    # Empty means no proxy domain, so operations use the wizard-api relay.
    domain: str = ""
    _client: "WizardClient" = field(default=None, repr=False, compare=False)
...
class _Sandboxes:
    def __init__(self, client: "WizardClient"):
        self._client = client
    ...
    def create(
        ...
        converts), template (a wizard template by id or name), or snapshot (a
    ...
    def subscribe(
        ...
        try:
            ...
            try:
                ...
                raise WizardAPIError(err["code"], err["message"], e.code) from None
            except (KeyError, ValueError):
                raise WizardAPIError("HTTP_ERROR", raw.decode(errors="replace"), e.code) from None
        except urllib.error.URLError as e:
            raise WizardConnectionError(f"{self._client.base_url}: {e.reason}") from None
        except (TimeoutError, OSError) as e:
            raise WizardConnectionError(f"{self._client.base_url}: {e}") from None
...
class WizardClient:
    def __init__(
        ...
        self.api_key = api_key or os.environ.get("WIZARD_API_KEY", "")
        self.base_url = (base_url or os.environ.get("WIZARD_BASE_URL", "http://127.0.0.1:8080")).rstrip("/")
        # proxy_url (or WIZARD_PROXY_URL) opts into the data plane: exec and file ops
        # go straight to the agent through wizard-proxy (Connect HTTP/JSON, no grpc
        # dependency) instead of relaying through wizard-api. Unset keeps the relay,
        # so nothing changes for a single-node/dev setup.
        self.proxy_url = proxy_url if proxy_url is not None else os.environ.get("WIZARD_PROXY_URL", "")
    ...
    def _request_raw(self, method: str, path: str, body: Optional[bytes] = None) -> bytes:
        ...
        try:
            ...
            raise WizardConnectionError(f"{self.base_url}: {e.reason}") from None
        except (TimeoutError, OSError) as e:
            raise WizardConnectionError(f"{self.base_url}: {e}") from None
