...
全自研栈：control plane（bean-api,内含 scheduler）、node daemon（noded）、in-sandbox daemon（beand）、proxy、SDK、CLI，
...
| [docs/noded-design.md](docs/noded-design.md) | node daemon：Runtime 抽象、镜像缓存、网络编排、reconcile;beand：PID1 注入、exec/PTY/文件 |
...
├── cmd/                    # bean-api / bean-proxy / noded / beand
├── internal/               # control (api/scheduler/store) / node (noded) / beand
├── pkg/                    # 对外 Go SDK（规划）
