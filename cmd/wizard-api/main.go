// wizard-api is the REST gateway. It serves the user-facing API and
...
import (
	...
	"github.com/garysng/wizard/internal/control/api"
	"github.com/garysng/wizard/internal/control/image"
	"github.com/garysng/wizard/internal/control/nodesvc"
...
func main() {
	...
	dbPath := flag.String("db", "wizard.db", "SQLite database path")
	postgresDSN := flag.String("postgres", os.Getenv("WIZARD_POSTGRES_DSN"),
		"Postgres connection string (or WIZARD_POSTGRES_DSN); overrides --db when set. "+
			"This is what allows more than one wizard-api replica: SQLite is a single file "+
			"and two replicas cannot share it, so on SQLite the count is exactly one")
	apiKey := flag.String("api-key", os.Getenv("WIZARD_API_KEY"), "API key (or WIZARD_API_KEY env)")
	...
	sandboxDomain := flag.String("sandbox-domain", os.Getenv("WIZARD_SANDBOX_DOMAIN"),
		"wizard-proxy public base stamped on each sandbox as its data-plane domain "+
	...
	secretKey := flag.String("secret-key", os.Getenv("WIZARD_SECRET_KEY"),
	...
	ownerHeader := flag.String("owner-header", "",
		"request header naming the caller an image is attributed to, e.g. "+
			"X-Wizard-Owner. Empty leaves every image unowned and every listing "+
			"unfiltered. Only set this behind a trusted layer that authenticates "+
			"callers and sets the header itself: wizard does not verify it, so a "+
	...
	s3Endpoint := flag.String("s3-endpoint", os.Getenv("WIZARD_S3_ENDPOINT"),
		"S3-compatible endpoint for snapshot blobs (or WIZARD_S3_ENDPOINT); empty uses --snapshot-dir")
	s3Bucket := flag.String("s3-snapshot-bucket", "wizard-snapshots",
		"bucket holding snapshot blobs")
	s3LogsBucket := flag.String("s3-logs-bucket", envOr("WIZARD_S3_LOGS_BUCKET", "wizard-build-logs"),
		"bucket build logs are read from (or WIZARD_S3_LOGS_BUCKET), the same dedicated "+
	...
	shutdownTracing, err := obs.SetupTracing(context.Background(), obs.TracingConfig{
		Endpoint: *otlpEndpoint, Service: "wizard-api", Version: version, Insecure: true,
	})
	if err != nil {
	...
	if *apiKey == "" {
		log.Fatal("api key required: set --api-key or WIZARD_API_KEY")
	}
	...
	if *postgresDSN != "" {
		...
		slog.Info("state store: sqlite", "path", *dbPath,
			"note", "one file, so exactly one wizard-api replica")
	}
	...
	grpcSrv := grpc.NewServer(
		grpc.ChainUnaryInterceptor(obs.UnaryServerTrace("wizard-api")),
		grpc.ChainStreamInterceptor(obs.StreamServerTrace("wizard-api")),
	)
	...
	slog.Info("wizard-api listening", "version", version, "http", *listen,
