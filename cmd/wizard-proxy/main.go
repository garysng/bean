// wizard-proxy routes traffic into sandboxes.
//
...
import (
	...
	"github.com/garysng/wizard/internal/control/proxy"
...
func main() {
	...
	controlPlane := flag.String("control-plane", "",
		"wizard-api base URL, e.g. http://wizard-api.internal:8080. Placement is read "+
	...
	apiKey := flag.String("api-key", os.Getenv("WIZARD_API_KEY"),
		"key this proxy authenticates to wizard-api with (or WIZARD_API_KEY). The proxy "+
	...
	nodeToken := flag.String("node-token", os.Getenv("WIZARD_NODE_TOKEN"),
		"token presented to a node's forwarding port (or WIZARD_NODE_TOKEN)")
	...
	if *controlPlane == "" {
		log.Fatal("--control-plane is required: the proxy asks wizard-api which node " +
			"holds a sandbox")
	}

	// No user authentication here, by design: an external layer does that, and wizard is
	...
	slog.Info("wizard-proxy listening", "version", version, "addr", *listen,
