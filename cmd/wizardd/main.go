// wizardd is the in-sandbox init/PID1 agent. On the fc tier it is
...
func fatalToConsole(format string, args ...any) {
	msg := fmt.Sprintf("wizardd: "+format+"\n", args...)
...
func main() {
	listenAddr := flag.String("listen", "/run/wizard/agent.sock", "unix socket path (or vsock:PORT on fc tier)")
	...
	if err := flag.CommandLine.Parse(os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr,
			"wizardd: ignoring unusable arguments (%v); this agent image predates a "+
	...
	if *pivot != "" {
		if err := wizardd.PivotToRootfs(*pivot); err != nil {
	...
	if *guestDNS != "" {
		if err := wizardd.WriteResolvConf(*rootDir, *guestDNS); err != nil {
			fatalToConsole("guest dns: %v", err)
		}
		slog.Info("guest resolver configured", "nameserver", *guestDNS)
	}

	lis, err := wizardd.Listen(*listenAddr)
	if err != nil {
	...
	interceptors := []connect.Interceptor{wizardd.ConnectTraceLogging()}
	...
	if authRequired {
		interceptors = append(interceptors, wizardd.NewAuthenticator().Interceptor())
	}

	agent := wizardd.NewServer(version, *rootDir)
	path, handler := agentv1connect.NewAgentServiceHandler(
		wizardd.NewConnectServer(agent),
	...
	// knowledge), and wizardd.Serve hands those connections straight to the HTTP/2
	...
	slog.Info("wizardd listening", "version", version, "addr", *listenAddr, "root", *rootDir,
	...
	if err := wizardd.Serve(lis, h2cHandler, mux); err != nil {
