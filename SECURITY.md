...
form](https://github.com/garysng/wizard/security/advisories/new) rather than a
public issue.

## What wizard is and is not, today

wizard runs untrusted code behind a hardware boundary: one Firecracker microVM per
...
- **No jailer chroot.** The VMM is not confined by a `chroot` and has no device
  ...
  why it is not next. [#20](https://github.com/garysng/wizard/issues/20)
- **No per-port access control.** Any port on a sandbox is reachable by anything
  that can reach wizard-proxy, so do not give a sandbox a port it would not want
  its caller to see. [#50](https://github.com/garysng/wizard/issues/50)
